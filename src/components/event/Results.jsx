...
export default function Resultado({
  ...
  const successMessage =
    eventoAtual.successMessage ||
    (participante ? "Seu código de acesso" : null);
  ...
  const amigoNome = participante
    ? safeName(eventoAtual.sorteio[participante.nome])
    : null;
              ...
              {amigoPresentes.length > 0 && (
                <div className="mt-3 text-left">
                  <p className="text-sm text-gray-700 mb-1">
                    Sugestões do seu amigo:
                  </p>
                  <ul className="list-disc list-inside text-sm text-gray-700">
                    {amigoPresentes.map((pres, i) => (
                      <li key={i}>{pres}</li>
                    ))}
                  </ul>
                </div>
              ...
              participante.filhos.map((filho) => {
                const filhoNome =
                  typeof filho === "string"
                    ...
                    ? filho.nome
                    : String(filho);
                const amigoDoFilhoNome = safeName(
                  eventoAtual.sorteio[filhoNome]
                );
                const amigoDoFilhoObj = amigoDoFilhoNome
                  ? findPersonByName(amigoDoFilhoNome)
                  : null;
                ...
                return (
                  ...
                  >
                    <p className="text-sm text-gray-600 mb-2">
                      Amigo secreto de <b>{filhoNome}</b>:
                    </p>
                    ...
                    {amigoDoFilhoPresentes.length > 0 && (
                      <div className="mt-3 text-left">
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-700">
                          {amigoDoFilhoPresentes.map((pres, i) => (
                            <li key={i}>{pres}</li>
                          ))}
                        </ul>
                      </div>
